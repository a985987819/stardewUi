...
const components = [
  ...
  },
  {
    ...
    title: 'Card 卡片',
    description: '支持标题区、配色和交互状态的卡片容器组件。',
    icon: <Square size={20} />,
  },
  {
    path: '/components/dialog',
    title: 'Dialog 对话框',
    description: '支持分页和打字机效果的对话框组件。',
...
function StarComponentsPage() {
  return (
    <div className={styles['components-page']}>
      <Card className={styles['components-header-card']}>
      ...
      <div className={styles['components-grid']}>
        {components.map((item) => (
          <Link key={item.path} to={item.path} className={styles['components-card-link']}>
            <Card className={styles['components-card']} hoverable>
