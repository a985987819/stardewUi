...
const loadingApiData = [
  ...
  { property: 'text', description: '加载文案；留空字符串时只显示图形，不传时使用默认文案与动态省略号', type: 'string', default: "'请稍候...'" },
  ...
  { property: 'fill', description: '是否占满父容器并在其中居中，适合页面区域加载态', type: 'boolean', default: 'false' },
...
const buttonCode = `<StarNineSliceButton loading variant="primary">
  保存中
</StarNineSliceButton>`
...
function StarLoadingDemoPage() {
  ...
  return (
    ...
    >
        ...
        <div className={styles['loading-demo__stack']}>
          <div className={styles['loading-demo__panel']}>
            ...
            <div className={styles['loading-demo__button-row']}>
              <StarNineSliceButton size="small" variant={active ? 'danger' : 'primary'} onClick={() => setActive((value) => !value)}>
                {active ? '停止动画' : '重新开始'}
              </StarNineSliceButton>
            </div>
      ...
      >
        <div className={styles['loading-demo__button-row']}>
          <StarNineSliceButton variant="primary" loading={buttonLoading}>
            保存中
          </StarNineSliceButton>
